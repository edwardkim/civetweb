...
6. 원격 게시 브랜치 push
   ...
   ```
7. master 대상 Open PR 생성
   ```bash
   WORKFLOW_SHA=$(git -C ../civetweb-workflow rev-parse HEAD)
   ...
   - 작업 문서는 `WORKFLOW_SHA` 기준 `https://github.com/edwardkim/civetweb/blob/{WORKFLOW_SHA}/mydocs/...` URL로 연결
