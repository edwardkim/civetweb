...
1. 중복 이슈 확인
   ```bash
   gh issue list --repo civetweb/civetweb --state all \
     --search "{작업 키워드}" \
     --limit 20 \
...
2. 열린 milestone 목록 확인
   ```bash
   gh api repos/civetweb/civetweb/milestones \
...
3. 기존 label 목록 확인
   ```bash
   gh api repos/civetweb/civetweb/labels --paginate \
...
8. 승인 후 이슈 생성
   ```bash
   gh issue create --repo civetweb/civetweb \
     --title "{제목}" \
     --body "{본문}" \
     --milestone "{milestone}" \
     --label "{label}"
...
9. 생성 결과 확인
   ```bash
   gh issue view {N} --repo civetweb/civetweb \
     --json number,title,state,milestone,labels,url
   ```
