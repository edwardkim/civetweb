...
이 문서는 `civetweb` 저장소의 내부 task PR 생성 규칙과 외부 기여자 PR 검토 절차를 정리한다.
...
내부 task PR은 `local/task{번호}`에서 작업한 내용을 `publish/task{번호}`로 push한 뒤 `master` 대상으로 생성한다.

PR 본문은 `.github/pull_request_template.md`를 기준으로 작성한다. 템플릿은 이 저장소의 하이퍼-워터폴 산출물과 연결되도록 확장한 형식이다.
...
- 작업 문서 링크는 `personal/hyper-waterfall`의 문서 commit SHA 기준 GitHub blob URL을 사용하고, raw URL 대신 `[파일명](URL)` 형식으로 표시한다.
...
- **[Stage 1](https://github.com/edwardkim/civetweb/blob/{sha}/mydocs/working/task_m050_22_stage1.md)** ([abc1234](https://github.com/edwardkim/civetweb/commit/{stage1_sha})): {Stage 1 요약}
- **[Stage 2](https://github.com/edwardkim/civetweb/blob/{sha}/mydocs/working/task_m050_22_stage2.md)** ([def5678](https://github.com/edwardkim/civetweb/commit/{stage2_sha})): {Stage 2 요약}
...
- 수행 계획서: [task_m050_22.md](https://github.com/edwardkim/civetweb/blob/{sha}/mydocs/plans/task_m050_22.md)
- 구현 계획서: [task_m050_22_impl.md](https://github.com/edwardkim/civetweb/blob/{sha}/mydocs/plans/task_m050_22_impl.md)
- 최종 보고서: [task_m050_22_report.md](https://github.com/edwardkim/civetweb/blob/{sha}/mydocs/report/task_m050_22_report.md)
